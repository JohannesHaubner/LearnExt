...
def compute_optimal_coefficient(mesh, V, Vs, params, deformation, def_boundary_parts,
    ...
    E = inner((1.0 + alpha) * (grad(u) + grad(u).T), grad(v) + grad(v).T) * dx(mesh)
    ...
    # solve u_opt
    E = inner((1.0 + alpha_opt) * (grad(u) + grad(u).T), grad(v)) * dx(mesh)
    solve(E == 0, u, bc)
    ...
    xdmf.write_checkpoint(alpha_opt, "alpha_opt", 0, append=True)
